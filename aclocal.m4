...
AC_COMPILE_IFELSE(
  ...
  [AC_MSG_RESULT(yes)
   AC_DEFINE([PAGESIZE_VIA_SYSCONF],[1],[pagesize available via sysconf])],
  [AC_MSG_RESULT(no)])
...
#include <sys/sysctl.h>]],
  ...
  [AC_MSG_RESULT(yes)
   AC_DEFINE([PAGESIZE_VIA_SYSCTL],[1],[pagesize available via sysctl])],
  [AC_MSG_RESULT(no)])
...
main () {
  ...
  return 0;
                  ...
                  [cvx_func_fopen_$1=yes],
                  [cvx_func_fopen_$1=no],
                  [cvx_func_fopen_$1=no])])
if test x$cvx_func_fopen_$1 = xyes; then
  AC_DEFINE([$2], 1,
